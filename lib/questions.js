...
document.addEventListener('DOMContentLoaded', function () {
   ...
   /* document.getElementById('create').addEventListener('click', function (event) {
    ...
    document.getElementById('create').addEventListener('click', async function (event) {
        ...
        const draftTableRows = document.querySelectorAll('#draftTable tbody tr');
        
        if (draftTableRows.length === 0) {
            alert('The draft is empty.');
            return; // Stop execution if there are no draft items
        }
    
        const draftData = Array.from(draftTableRows).map(row => {
            const questionText = row.cells[0].textContent;
            const answerText = row.cells[1].textContent;
            const contextText = row.cells[2].textContent; // Assuming format "DOC docId:paragId,paragId; DOC docId:paragId"
    
            // Parse contextText to contextData format
            const contextData = contextText.split('; ').map(docInfo => {
                const [docIdPart, paragIds] = docInfo.trim().split(':');
                const docId = docIdPart.replace('DOC ', '').trim();
                return paragIds.split(',').map(paragId => ({
                    docId: parseInt(docId, 10),
                    paragId: parseInt(paragId, 10)
                }));
            }).flat();
    
            return { questionText, answerText, contextData };
        });
    
        // Create a function that returns a fetch promise
        const sendDraftData = async (data) => {
            const response = await fetch(`${api}/create-question-answer`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        try {
            ...
            const results = await Promise.all(draftData.map(sendDraftData));
    
            // Process the results
            results.forEach((result, index) => {
                if (result.success) {
                    console.log(`Question and answer ${index + 1} saved successfully.`, result);
                    openModal(`Question and answer ${index + 1} saved successfully.`);
                } else {
                    console.error(`Failed to save question and answer ${index + 1}.`, result);
                    openModal(`Failed to save question and answer ${index + 1}. Reason: ` + result.message);
                }
            });
        } catch (error) {
            console.error('Error:', error);
            openModal('An error occurred while saving the questions and answers.');
        }
    });
